...
void criar_t(){
	...
	printf("\n");
	tipo_var->nome_arq = malloc(sizeof(char)*105);
	strcpy(tipo_var->nome_arq,tipo_var->nome_t);//nome_arq recebe o nome da nova tabela
	strcat(tipo_var->nome_arq,".txt");//nome_arq recebe ela mesma + string ".txt"
	arquivo = fopen(tipo_var->nome_arq,"a");//abre arquivo nome_arquivo para anexar, caso ainda não exista é criado e se existe, o texto
	...
	if(arquivo == NULL){//caso dê erro na abertura do arquivo, exibe a seguinte mensagem e sai do programa
		...
		printf("\n");
		fprintf(arquivo,"|%s <1>|",tipo_var->nome_c);
		...
		while(opc == 's'){
			...
			printf("\n");
			fprintf(arquivo,"%s <%d>|", tipo_var->nome_c, tipo_c);		
